 ...
 * Tagline:  "Direct from Factory & Wholesaler" (opt-in via showTagline)
 */
...
const SIZES = {
  sm:  { icon: 40, name: 'text-[18px]', tagline: 'text-[8px]',  gap: 'gap-2'   },
  md:  { icon: 42, name: 'text-[20px]', tagline: 'text-[9px]',  gap: 'gap-2.5' },
  lg:  { icon: 52, name: 'text-[26px]', tagline: 'text-[11px]', gap: 'gap-3'   },
} as const

export function ThokSaleLogo({ size = 'md', showTagline = false, className = '', dark = false }: ThokSaleLogoProps) {
  const s = SIZES[size]
